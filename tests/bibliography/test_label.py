...
def test_bib_no_page_master116(td, mp):
    nopages = run_label(hoverpower.MASTER116_PDF, mp, td, {6061})
    expected_intext = 82
...
def test_bib_no_page_master98(td, mp):
    nopages = run_label(hoverpower.MASTER098_PDF, mp, td, {6061})
...
def test_bib_label_exists(td, mp):
    missing = run_label(hoverpower.MASTER116_PDF, mp, td, {6050})
...
def test_bib_source_not_required(td, mp):
    notrequired = run_label(hoverpower.MASTER116_PDF, mp, td, {6051})
    assert not notrequired  # TODO: VALIDATE LATER


def test_bib_label_improvement(td, mp):
    improvement = run_label(hoverpower.MASTER091B_PDF, mp, td, {6070})
    assert improvement
...
def test_regression_bachelor75(td, mp):
    ...
    linting = run_label(hoverpower.BACHELOR075_PDF, mp, td, {6061})
...
def test_label_bib_not_required(td, mp):
    ...
    linting = run_label(hoverpower.BACHELOR075_PDF, mp, td, {6051})
    assert not linting
...
def test_regression_bachelor90(td, mp):
    ...
    """
    error = run_label(hoverpower.MASTER091B_PDF, mp, td, {6050})
    assert not error
...
def test_bib_source_not_found_bachelor56_page6(td, mp):
    ...
    behavior is fixed."""
    bibliography = tests.bibliography.load_bib_table(hoverpower.BACHELOR056_PDF)
...
def test_diss143_numbered_label_6050(td, mp):
    error = run_label(hoverpower.DISS143_PDF, mp, td, {6050})
    assert not error


def test_diss143_add_pagination_hint(td, mp):
    hint = run_label(hoverpower.DISS143_PDF, mp, td, {6063})
    assert len(hint) == 1
...
def test_reg_bib_not_required_diss172(td, mp):
    ...
    # TODO: NOT READY YET
    linting = run_label(hoverpower.DISS172_PDF, mp, td, {6051})
    assert not linting
